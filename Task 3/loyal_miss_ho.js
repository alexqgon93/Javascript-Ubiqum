let least_loyalty_senate=document.getElementById("least_loyalty_house");
let top_loyalty_senate=document.getElementById("top_loyalty_house");
...
for(let i=0; i<percentage ;i++){
    template_t+=`<tr>
                ...
                top_loyalty_house.innerHTML=template_t;
}
let s_m_v_rev=s_m_v.reverse();
for(let i=0; i<percentage; i++){
        template_l+=`<tr>
                        <td><a href="${s_m_v_rev[i].url}">${s_m_v_rev[i].first_name} ${s_m_v_rev[i].middle_name||""} ${s_m_v_rev[i].last_name}</a></td>
                        <td>${s_m_v_rev[i].missed_votes}</td>
                        <td>${s_m_v_rev[i].missed_votes_pct}</td>
                    </tr>`;
                    least_loyalty_house.innerHTML=template_l;
}